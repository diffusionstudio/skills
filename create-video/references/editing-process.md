...
- **Context survives.** Footage analysis and shortlists are long; offloading
...
Inventory everything `dapi asset ls` returns. One row per asset: id, type,
name, duration, and a one-line "what it is" (fill the last column as you
analyze).
...
Go through each asset with the inspection tools (see `inspection.md`) and
record findings: what happens, usable in/out ranges, cut points, audio quality,
speech.

- `visualize` first (fast, free) for a filmstrip + waveform overview of each clip.
- `frame` to pin exact transition/cut timestamps.
- `transcribe` for any clip with speakers (word-level times for cutting on a
  line). `analyze` only when meaning matters and speech is sparse or absent;
...
or hides jump cuts. List candidates against the gaps from step 3. If you don't
have coverage, this is where you decide what to **AI-generate** (see
`choosing-commands.md`, Generation); a shortlist of prompts is itself a
plannable artifact.
...
module you feed `mount` (timing maps directly to
`inPoint`/`outPoint`/`startTime`; read the API reference's `jsx/timing.md`).
...
- **Footage-first** (above): you have the material, so map → analyze →
  shortlist → plan. Best for editing existing footage.
- **Brief-first:** start from a creative brief (`edit/brief.md`) with goal,
  audience, tone, length, and beats, *then* find or generate footage to satisfy
  it. Best when generating most assets from scratch. Either way the brief/plan
  is a written artifact you assemble from.

## Then assemble

Only once the plan exists do you build: author the project module (keep it next
to the plan, e.g. `edit/project.tsx`, and read the JSX reference pages that
match what you're writing; see SKILL.md), place clips with timing straight from
the plan, and **verify each step visually** (`inspection.md`). Re-`mount` as
the module evolves; keyed roots rebuild in place. Update the plan file as
reality diverges; it stays the source of truth for the edit.
