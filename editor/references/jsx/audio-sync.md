...
`syncTo` places a node in time by listening instead of arithmetic: the node's audio is cross-correlated against another node's audio, and its `startTime` is computed so the two recordings coincide on the timeline. It replaces manual offset measurement for multi-recorder material: a lav or voice track against camera audio, two cameras on the same take, two microphones in one room.
...
- The computed placement is `startTime = target.startTime + offset`, where `offset` is the measured source-time offset between the two recordings (positive when this node's recording started after the target's; possibly negative). `syncTo` and `startTime` are mutually exclusive.
- `inPoint`/`outPoint` keep their normal meaning and remain yours to set. When omitted on a synced node, the window defaults to the intersection of the node's natural extent with the target's window (instead of the usual natural-duration fit), so a lav track simply covers its take.
...
- Chains resolve in dependency order (A may sync to B while B syncs to C). Unknown keys, cycles, and combining `syncTo` with `startTime` are mount errors; nothing is inserted.
